...
void PrintPDFPageCount(const char* filename) {
...
CGPDFPageRef GetPDFPageFromFile(const char* filename, size_t pageNumber) {
    ...
    // Исправление 1: Добавлена закрывающая скобка
    ...
    // Исправление 2: Правильное имя переменной
...
void ExtractPDFMetadata(const char* filename) {
...
bool IsPDFEncrypted(const char *filename) {
    // Создаем CFURL из имени файла
    CFStringRef filenameStr = CFStringCreateWithCString(kCFAllocatorDefault,
                                                      filename,
                                                      kCFStringEncodingUTF8);
...
void CheckPDFEncryption(const char *filename) {
    bool encrypted = IsPDFEncrypted(filename);
    printf("PDF '%s' is %sencrypted\n",
           filename,
           encrypted ? "" : "not ");
}

void CheckPDFVersion(const char* filename) {
    // Создаем CFURL из имени файла
    CFStringRef filenameStr = CFStringCreateWithCString(kCFAllocatorDefault,
                                                     filename,
                                                     kCFStringEncodingUTF8);
    ...
    // Исправление 2: Объявление переменных
...
void ProcessOCG(CGPDFDictionaryRef ocgDict, int level);

void CheckPDFLayers(const char* filename) {
...
bool VerifyPDFIntegrity(const char* filename) {
    // Создаем CFURL из имени файла
    CFStringRef filenameStr = CFStringCreateWithCString(kCFAllocatorDefault,
                                                     filename,
                                                     kCFStringEncodingUTF8);
...
bool DeletePDFFile(const char* filename) {
...
bool DuplicatePDFFile(const char* originalPath) {
    // Проверка входных параметров
...
void performPDF(void) {
    ...
    CGPDFDocumentRef doc = MyGetPDFDocumentRef(filename);
    
    CGPDFPageRef page = CGPDFDocumentGetPage(doc, 1);
    if (page) {
        checkFontsOnPage(page);
    }
    
    CGPDFDocumentRelease(doc);
    ExtractPDFMetadata(filename);
    printPDFContent(filename);
    CheckPDFLayers(filename);
    VerifyPDFIntegrity(filename);
//    DeletePDFFile(filename);
    DuplicatePDFFile(filename);
}
